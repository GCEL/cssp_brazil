...
ci_names <- c('2pt5','mean','97pt5')
var_spadis_names <- c('gpp','cue','npp','nppwood','outputwood','wood','mrtwood')
...
for (e in ecoregion_names) {
  for (i in cardamom_model_exp) {
    ...
    model_reg_par_mean_spadist_df <- data.frame(matrix(nrow=35,ncol=4))
    colnames(model_reg_par_mean_spadist_df) <- cardamom_model_exp
    model_reg_par_mean_spadist_df$vars <- rep(var_spadis_names,length(ecoregion_names))
    model_reg_par_mean_spadist_df$region <- c(rep(ecoregion_names[1],length(var_spadis_names)),
...
# model_reg_par_mean_spadist_df$region <- rep(e,nrow(model_reg_par_mean_spadist_df))
...
gpp_spadis<- round(c(gpp_list_ci[,1],gpp_list_mean[1],gpp_list_ci[,2]),digits=2)
...
cue_spadis<- round(c(cue_list_ci[,1],cue_list_mean[1],cue_list_ci[,2]),digits=3)
...
npp_spadis<- round(c(npp_list_ci[,1],npp_list_mean[1],npp_list_ci[,2]),digits=2)
...
nppwood_spadis<- round(c(nppwood_list_ci[,1],nppwood_list_mean[1],nppwood_list_ci[,2]),digits=2)
...
outputwood_spadis<- round(c(outputwood_list_ci[,1],outputwood_list_mean[1],outputwood_list_ci[,2]),digits=2)
...
wood_spadis<- round(c(wood_list_ci[,1],wood_list_mean[1],wood_list_ci[,2]),digits=2)
...
mrtwood_spadis<- round(c(mrtwood_list_ci[,1],mrtwood_list_mean[1],mrtwood_list_ci[,2]),digits=2)
...
all_reg_par_mean_spadist_df <- rbind(C_northwest_par_mean_spadist_df,
                                     R2_southwest_par_mean_spadist_df,
                                     R2_brazilshield_par_mean_spadist_df,
                                     R2_eastcentral_par_mean_spadist_df,
                                     R2_guyanashield_par_mean_spadist_df)
all_reg_par_mean_spadist_df

write.csv(all_reg_par_mean_spadist_df,"all_reg_par_mean_spadist_df.csv")
