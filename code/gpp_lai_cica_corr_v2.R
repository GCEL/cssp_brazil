...
amazon_reg_to_card_var_2 <- function(s,s2,t) {
  r <- brick('R://ILAMB_beta_devel/RAINFOR_leeds_run/cssp_brazil_amazon_run/DATA/benchmark/NoRainfor_Amazon_1deg_monthly_2001_2019_compare_w_CiCa.nc',varname=t)
  a <- r$X2001.01.01
...
amazon_to_card_var_2 <- function(s2,t) {
  card_region <- brick('R://ILAMB_beta_devel/RAINFOR_leeds_run/cssp_brazil_amazon_run/DATA/benchmark/NoRainfor_Amazon_1deg_monthly_2001_2019_compare_w_CiCa.nc',varname=t)
  df_card_region <- as.data.frame(card_region,xy=F)
  df_card_region<-as.data.frame(t(df_card_region))
  df_card_region <- data.frame(y=unlist(df_card_region))
  names(df_card_region) <- paste(tolower(t))
...
reg_card_var_2 <- function(s,s2,cardamom_variables){
  ...
  date<- as.Date(rep(seq(from = as.Date("2001-01-01"), to = as.Date("2019-12-01"), by = 'month'),1116))
  month<-format(date,format="%b")
  year <- format(date,format="%Y")
  
  return(na.omit(cbind(gpp_reg_data,lai_reg_data,cica_reg_data,region_name,date,month,year)))
}
...
all_reg_run_2 <- function(s,s2,cardamom_variables) {
  for (i in s2) {
    if(i=='amazon_nw'){
      ...
      amazon_data <- card_var_2(i,var_names)
    }
  }
